...
sidebarTitle: "Test Automation with CI/CD"
...
keywords: ['testmu ai automation', 'testmu ai kaneai', 'kaneai guided walkthrough', 'kane ai test run', 'hyperexecute', 'test run', 'test manager', 'ci/cd', 'jenkins', 'github actions', 'api', 'postman']
"og:description": "Automate test run execution on KaneAI with CI/CD tools like GitHub Actions and Jenkins. Follow our step-by-step guide for seamless integration."
---
...
<img loading="lazy" src='/assets/images/kane-ai/test-manager/test-plan-ci-cd/step1.png' alt="Image" />
...
- **tunnel**: Add the parameter and tunnel name if you want to run using <BrandName /> Tunnel for private applications. See more details for tunnel [here](/docs/kane-ai-geolocation-tunnel-proxy/#tunnel-support).
...
- **geolocation**: Add the resgion of the geolocation that you want to run your tests from. You can find the list of support geolocations [here](/docs/selenium-geolocation-capabilities/).
- **environment_id**: Define the environment on which you want to run the test run if required.
- **retry_on_failure**: Define whether to retry if the [testRunnerCommand](/docs/deep-dive-into-hyperexecute-yaml/#testrunnercommand) execution fails. If "retry_on_failure" is false, the default max retries will still be 1. You can find more details [here]( https://www.lambdatest.com/docs/deep-dive-into-hyperexecute-yaml/#retryonfailure).
- **max_retries**: Define the number of maximum retries you want (Max value 5). Default retries 1. You can find more details [here](/docs/deep-dive-into-hyperexecute-yaml/#maxretries). 
- **timezone**: Define the timezone you want to choose for test run in UTC+01:00 format.
- **app_profiling**: Add true if you want to track app profiling metrics in your test. You can find more details [here](/docs/appium-app-performance-analytics/).
- **performance**: Add true if you want generate Lighthouse report for your web tests. Supported on limited OS browser combinations. Additionally, Setting this as true could potentially slow down the execution time. You can find more details [here](/docs/view-lighthouse-performance-metrics/).
...
<img loading="lazy" src='/assets/images/kane-ai/test-manager/test-plan-ci-cd/image7.png' alt="Image" />
...
<source src= '/assets/images/kane-ai/test-manager/test-plan-ci-cd/output.mp4' type="video/mp4" />
</video>
