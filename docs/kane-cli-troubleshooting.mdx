...
keywords: ['kane cli troubleshooting', 'kaneai errors', 'testmu ai', 'chrome failed to launch', 'authentication failed']
"og:description": "Fix common Kane CLI issues: Chrome launch failures, authentication errors, run timeouts, variables not resolving, upload failures, and Agent Mode output problems."
---
...
4. Pick a different Chrome user-data directory, or quit the Chrome instance using it. See [Chrome Management](/docs/kane-cli-configuration/#chrome-management)
...
2. **File location.** Confirm your file is in the right place — see [loading order](/docs/kane-cli-variables-and-context/#loading-order).
...
The notice is informational — your current version still works. To upgrade, follow the steps in [Updates](/docs/kane-cli-installation/#update).
