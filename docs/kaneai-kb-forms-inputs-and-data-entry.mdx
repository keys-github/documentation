...
sidebarTitle: "Forms, Inputs & Data Entry"
---
...
<Tip>
Parameters once used in authoring can be used to input different data values via dataset. See the [Parameters](/docs/kane-ai-using-parameters/) guide for more information.
</Tip>
...
<Tip>
Smart variables like `{{smart.random_email}}` generate unique values every run, which is ideal for registration flows where duplicate emails would fail. See the [Smart Variables](/docs/kane-ai-smart-variables/) guide for the full list.
</Tip>
...
<Tip>
**JS Workaround:** If the date picker uses a read-only input that doesn't accept typed values, use a JS snippet to set the value directly:
...
<Note>
For detailed instructions, refer to [File Upload & Download Testing](/docs/kaneai-upload-and-download-files).
</Note>
## OTP & PIN Fields
...
For real TOTP-based MFA, see the [TOTP Authentication](/docs/kaneai-totp) guide.
