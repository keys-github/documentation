...
"og:description": "Learn Puppeteer Visual Regression Set up SmartUI, run tests on TestMu AI Cloud across 40+ browsers. Step-by-step guide for seamless testing."
---
...
(async () => {
  ...
  try {
    ...
    `lambdatest_action: ${JSON.stringify({ action: "smartui.takeScreenshot", arguments: { fullPage: true, screenshotName: "Navigating to <BrandName />" } })}`);
...
For additional information about Puppteer framework please explore the documentation [here](/support/docs/puppeteer-testing/)
