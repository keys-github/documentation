...
"og:description": "In this documentation, learn how integrate your Selenium C# automated tests with TestMu AI's SmartUI."
---
...
You can clone the sample repository to run `<BrandName />` automation tests with `SmartUI` and use `LTCloudTest.cs` file located in the `<BrandName />.Selenium.Driver.Test` folder.
  
```bash
git clone https://github.com/LambdaTest/smartui-csharp-sample
cd smartui-csharp-sample/<BrandName />.Selenium.Driver.Test
```
...
```cs
<ItemGroup>
    <PackageReference Include="<BrandName />.Selenium.Driver" Version="1.0.4" />
</ItemGroup>
```

<Note>
**You can check the latest version of [<BrandName />.Selenium.Driver]( https://www.nuget.org/packages/<BrandName />.Selenium.Driver) and update the latest version accordingly.**
...
using <BrandName />.Selenium.Driver;

namespace <BrandName />.Selenium.TestProject
{
...
<Tab title="Element CSS Selector">
...
<Tab title="NuGet Dependencies Not Resolving">

**Issue: NuGet Dependencies Not Resolving**

**Symptoms**: NuGet cannot find `<BrandName />.Selenium.Driver` package
...
**Solutions**:
1. Check latest version on [NuGet.org](https://www.nuget.org/packages/<BrandName />.Selenium.Driver)
2. Clear NuGet cache:
   ```bash
