...
You can clone the sample repository to run `<BrandName />` automation tests with `SmartUI` and use the `sdkCloud.rb` file present in the `sdk` folder.
...
```
gem install testmu ai-selenium-driver selenium-webdriver
```
...
require "testmu ai/selenium/driver"

driver = Selenium::WebDriver.for :chrome
driver.navigate.to "https://www.pinterest.com/pin/16958936087791895/"

# Take a snapshot
<BrandName />::Selenium::Driver.smartui_snapshot(driver, "Screenshot Name")
...
<BrandName />::Selenium::Driver.smartui_snapshot(driver, "Screenshot Name", options)
```
...
**Example:**
```ruby
<BrandName />::Selenium::Driver.smartui_snapshot(driver, HomePage-Header")
<BrandName />::Selenium::Driver.smartui_snapshot(driver, "ProductPage-MainContent")
```
...
<BrandName />::Selenium::Driver.smartui_snapshot(driver, Page Loaded")
```
...
<Tab title="Gem Dependencies Not Resolving">

**Issue: Gem Dependencies Not Resolving**

**Symptoms**: Bundler cannot find `testmu ai-selenium-driver` gem
...
5. Update Gemfile with specific version:
   ```ruby
   gem 'testmu ai-selenium-driver', '~"> 1.0'
   ```
