...
keywords: ['local app testing', 'testmu ai local testing', 'testmu ai local tunnel', '<BrandName /> tunnel', 'local app testing for linux', 'ssh connection', 'shared ssh', 'secure-shell-tunnel', 'docker compose ssh tunnel', 'docker vpn tunnel']
...
`docker pull testmu ai/tunnel:latest`

<img loading="lazy" src='/assets/images/docker-tunnel/raise-the-pull-request.webp' alt="Image" width="756" height="169" />

In order to pull the other docker images from the hub, use the below command:

* **Legacy Alpine:**
`docker pull testmu ai/tunnel:legacy-alpine`

* **Alpine:**
`docker pull testmu ai/tunnel:alpine`

* **Legacy:**
`docker pull testmu ai/tunnel:legacy`

* **Burrow:**
`docker pull testmu ai/tunnel:burrow`
...
`docker run -it  testmu ai/tunnel --version`

<img loading="lazy" src='/assets/images/docker-tunnel/version-tracking.webp' alt="Image" width="732" height="91" />

* **Basic Authentication and Login**: You can login into your <BrandName /> Tunnel with Docker image, with the help of below command:

`docker run -it --name lt testmu ai/tunnel --user johndoe --key XXXXXXXXXXXX`

<img loading="lazy" src='/assets/images/docker-tunnel/Basic-Authentication-and-Login.webp' alt="Image" width="1534" height="179" />

* **Capturing logs in mounted volume on host machine**: You can capture these logs with the help of below command:

`docker run -it  -v /mydir:/logs testmu ai/tunnel --user joendoe --key XXXXXXXX --logFile /logs/tunnel.log`

<img loading="lazy" src='/assets/images/docker-tunnel/Capturing-logs.webp' alt="Image" width="1540" height="169" />
...
`docker run -it -p 13001:8000 testmu ai/tunnel --user johndoe --key XXXXXXX  --infoAPIPort 8000`

<img loading="lazy" src='/assets/images/docker-tunnel/Getting-Tunnel-Status.webp' alt="Image" width="1534" height="176" />

Now, in order to fetch the current tunnel status, just execute the below command:

`curl http://127.0.0.1:13001/api/v1.0/info`

<img loading="lazy" src='/assets/images/docker-tunnel/docker-tunnel-status.webp' alt="Image" width="1533" height="168" />
...
`docker run -it testmu ai/tunnel --user johndoe --key XXXXXXX  --proxy-host host.docker.internal  --proxy-port 8082 --proxy-user foo --proxy-pass bar`
...
`docker run -it testmu ai/tunnel --user johndoe --key XXXXXXX  --proxy-host 172.17.0.2  --proxy-port 8082 --proxy-user foo --proxy-pass bar`
...
`docker run -it testmu ai/tunnel --user johndoe --key XXXXXXX  --proxy-host proxy-service  --proxy-port 8082 --proxy-user foo --proxy-pass bar`
...
<img loading="lazy" src='/assets/images/docker-tunnel/info-api.webp' alt="Image" width="909" height="167" />
...
`docker run -it  --network host testmu ai/tunnel --user johndoe --key XXXXXXXXXXXX`
