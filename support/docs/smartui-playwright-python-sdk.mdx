...
- You can clone the sample repository to run `TestMu AI` automation tests with `SmartUI` and use the `SmartUI_SDK_LT_hub.py` file.
...
```bash
pip install testmu ai-playwright-driver
```
...
**Symptoms**: pip cannot find `testmu ai-playwright-driver` or package installation fails
...
1. Check latest version:
   ```bash
   pip index versions testmu ai-playwright-driver
   ```
